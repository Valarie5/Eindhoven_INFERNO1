...
%Chebyshev compute Differentiation matrix, x=Chebyshev grid
function [D, x] = cheb(N)
  if N == 0
    D = 0; x = 1; return
  end
...
% Calculate error (reference: linear profile T* = y* for low Pe approximation)
  y_star = (Y + 1) / 2;  % Convert Chebyshev Y to y* [0,1]
  T_ref = y_star;  % Linear reference solution (T* = y* from 0 to 1)
  error = norm(T - T_ref, 2) / norm(T_ref, 2);  % Relative L2 norm error
  fprintf('Pe = %.1f, Relative Error = %.4e\n', Pe, error);
...
ylabel('Temperature');
legend({'$Pe = 0.1$','$Pe = 1.0$','$Pe = 10.0$', '$Pe = 100.0$'},'Interpreter','Latex') %Changeable
hold off;